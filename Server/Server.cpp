...
int  main(void) 
{
	...
	{
		// we won't log anything since it will be loggedmall		// by InitializeWindowsSockets() function
		return 1;
	}
	...
	// put listen socket in nonblocking mode
	if (!PutSocketInNonblockingMode(&listenSocket))
		return 1;

	printf("Server initialized, waiting for clients.\n");

	//int *closeServer = (int*)malloc(sizeof(int));
	...
	while (WaitForSingleObject(handleFinishSignal, 10) == WAIT_TIMEOUT)
	{
		...
		//PrintClientInformationList(&clientInformation);
